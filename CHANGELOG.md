...
**Kimi / DeepSeek / ChatGPT / Claude / 元宝 全部回归测试通过**(v1.18.0 production-ready 基础 + 2 个 list indent cascade fix 不影响其他站点,因为 fix 是通用 markdown 规范化层,所有站点的 `<ol>/<ul>/<li>` 都受益)。Last 3 个脚本版本:v1.19.0(2026-07-20, 豆包真正支持 + OL list continuation indent 修复, semver minor) / v1.18.0(2026-07-20, Kimi 输出 production-ready: 3 fix 链根治 setext h2 + 多块塌缩 + inline 丢失 + Kimi chrome 污染, semver minor) / v1.17.0(2026-07-19, Kimi 真正支持:修 v1.16.0 stub selector 3 处 + semver minor)。
